...
{
	m_bEditing = TRUE;
	CString timestg = GetText();
	COleDateTime dt;
	//CTime dt;
	if (dt.ParseDateTime(timestg)) 
	{
	    //COleDateTime CurrentCellTime(dt.GetYear(), dt.GetMonth(), dt.GetDay(), 0,0,0);
	    CTime CurrentCellTime(dt.GetYear(), dt.GetMonth(), dt.GetDay(), 0,0,0);
	    SetTime(CurrentCellTime);
	}
...
//void CGridCellDateTime::SetTime(COleDateTime time)
								   ...
								   UINT nFirstChar)
{
	...
	//COleDateTime theTime = *m_pcTime;
	CTime theTime = *m_pcTime;
	//SetTime(theTime);
	SetTime(m_pcTime);
